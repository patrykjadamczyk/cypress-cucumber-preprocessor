...
describe("getStepDefinitionsPaths", () => {
  ...
  it("should return the correct folder for script for scenario", () => {
    jest.resetModules();
    jest.mock("cosmiconfig", () => () => ({
      load: () => ({
        config: {
    ...
    const actual = getStepDefinitionsPaths("/path");
    const expected = "/path.+(js|ts)";

    expect(actual).to.include(expected);
  });
  it("should return the correct folder for script for scenario with extension", () => {
    jest.resetModules();
    jest.mock("cosmiconfig", () => () => ({
      load: () => ({
        config: {
    ...
    const actual = getStepDefinitionsPaths("/path.feature");
    const expected = "/path.+(js|ts)";
